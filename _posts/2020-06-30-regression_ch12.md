...
7. $$\rho=0$$ implies that the error terms are uncorrelated.
   > <U>Proof</U>
   > Note that $$\epsilon_t = \rho \epsilon_{t-1} + u_t = \rho^2 \epsilon_{t-2} + \rho u_{t-1}  + u_t$$ > $$= \rho^3 \epsilon_{t-3} + \rho^2 u_{t-2}  + \rho u_{t-1}  + u_t = \cdots =$$ **$$\sum_{s=0}^\infty \rho^s u_{t-s}$$**.
   >
...
i.e. $$Y_t = $$**$$\beta*0 + \beta_1 X_t$$** $$ + $$ **$$\rho \epsilon*{t-1}$$** $$ + $$ **$$u_t$$\*\*.
